...
static int CH347_Read(void *oBuffer, uint32_t *ioLength)
{
    ...
    LOG_DEBUG_IO("(size=%d, buf=[%s]) -> %" PRIu32, rlength, HexToString((uint8_t *)oBuffer, rlength), rlength);
    *ioLength = rlength;
    return true;
}
...
static int ch347_init(void)
{
    ...
    if (uhModule == 0) {
        ...
        if (uhModule) {
            upOpenDev = (pCH347OpenDevice)GetProcAddress(uhModule, "CH347OpenDevice");
            upCloseDev = (pCH347CloseDevice)GetProcAddress(uhModule, "CH347CloseDevice");
            upReadData = (pCH347ReadData)GetProcAddress(uhModule, "CH347ReadData");
            upWriteData = (pCH347WriteData)GetProcAddress(uhModule, "CH347WriteData");
            upSetTimeout = (pCH347SetTimeout)GetProcAddress(uhModule, "CH347SetTimeout");
            upJtagInit = (pCH347Jtag_INIT)GetProcAddress(uhModule, "CH347Jtag_INIT");
            if (upOpenDev == NULL || upCloseDev == NULL || upSetTimeout == NULL || upReadData == NULL || upWriteData == NULL || upJtagInit == NULL) {
